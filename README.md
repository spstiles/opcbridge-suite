...
- SIP callouts:
  - `pjsua` (pjproject) is installed only when explicitly requested with `--with-pjsip`, or when running `--deps` (as part of “install all dependencies”).
  - To skip building `pjsua` even when using `--deps`, pass `--no-pjsip`.
